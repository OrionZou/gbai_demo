...
async def run():
    ...
    try:
        obj = client.create_object(
            class_name=class_name,
            properties={"name": "iPhone 15", "price": 799, "brand": "Apple"},
        )
    ...
    # 5. 更新对象
    client.patch_object(object_id, class_name="Product", properties={"price": 749})
    print(client.get_object(object_id))

    result = client.get_object_vector(class_name=class_name, object_id=object_id)
    # print(result)
    query = "三星"
    query_embedding = await embed_client.embed_text(query)
    result = client.search(
        class_name=class_name, query=query, vector=query_embedding, fields=["brand"]
    )
    print(result)
