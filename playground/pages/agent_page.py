...
class AgentPage:
    ...
    def _load_agent_names(self) -> bool:
        ...
        if 'agent_names' not in st.session_state:
            try:
                response = requests.get(f"{self.api_url}/agents/names", timeout=10)
    ...
    def _load_agent_info(self, selected_agent: str) -> bool:
        """加载Agent详细信息"""
        try:
            response = requests.get(f"{self.api_url}/agents/{selected_agent}/prompts", timeout=10)
            if response.status_code == 200:
                agent_info = response.json()
                # 只在Agent变更时更新信息，避免重复请求
                if ('current_agent_info' not in st.session_state or
                        st.session_state.current_agent_info.get('agent_name') != selected_agent):
                    st.session_state.current_agent_info = agent_info
                    st.session_state.original_system_prompt = agent_info.get('system_prompt', '')
                    st.session_state.original_user_template = agent_info.get('user_prompt_template', '')
                return True
            else:
    ...
    def _render_operation_panel(self, current_system_prompt: str, current_user_template: str):
        """渲染操作面板"""
        st.markdown("#### 🔧 操作中心")
        
        # 检查是否有修改
        has_system_changes = current_system_prompt != st.session_state.get('original_system_prompt', '')
        has_template_changes = current_user_template != st.session_state.get('original_user_template', '')
        has_changes = has_system_changes or has_template_changes
        
        # 状态指示器
        self._render_change_status(has_changes, has_system_changes, has_template_changes)
        
        # 主要操作按钮
        self._render_main_operations(has_changes, current_system_prompt, current_user_template)
        
        # 辅助操作按钮
        self._render_auxiliary_operations()
        
        # 内容统计
        self._render_content_stats(current_system_prompt, current_user_template, has_changes)
    
    def _render_change_status(self, has_changes: bool, has_system_changes: bool, has_template_changes: bool):
        """渲染变更状态"""
        if has_changes:
            st.warning("⚠️ 有未保存修改")
            changes_info = []
    ...
    def _render_main_operations(self, has_changes: bool, current_system_prompt: str, current_user_template: str):
        """渲染主要操作按钮"""
        ...
        try:
            with st.spinner("正在保存..."):
                response = requests.put(
                    f"{self.api_url}/agents/{selected_agent}/prompts",
                    json=update_data,
                    timeout=30
                )
            ...
            if response.status_code == 200:
                ...
                st.session_state.original_system_prompt = updated_info.get('system_prompt', '')
                st.session_state.original_user_template = updated_info.get('user_prompt_template', '')
                st.success("✅ 保存成功！")
                st.rerun()
            else:
    ...
    def _execute_reset(self):
        ...
        selected_agent = st.session_state.selected_agent_name
        try:
            response = requests.post(f"{self.api_url}/agents/{selected_agent}/prompts/reset", timeout=30)
            if response.status_code == 200:
                reset_info = response.json()
                st.session_state.current_agent_info = reset_info
                st.session_state.original_system_prompt = reset_info.get('system_prompt', '')
                st.session_state.original_user_template = reset_info.get('user_prompt_template', '')
                st.success("✅ 已重置")
                st.rerun()
            else:
                st.error("重置失败")
    ...
    def _execute_refresh(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                st.session_state.original_system_prompt = fresh_info.get('system_prompt', '')
                st.session_state.original_user_template = fresh_info.get('user_prompt_template', '')
                st.success("✅ 已刷新")
                st.rerun()
            else:
                st.error("刷新失败")
    ...
    def _render_content_stats(self, current_system_prompt: str, current_user_template: str, has_changes: bool):
        """渲染内容统计"""
        st.markdown("---")
        st.markdown("##### 📊 内容统计")
        ...
        if template_vars:
            ...
            with st.form("quick_validate_form"):
                ...
                self._handle_validation_buttons(
                    validate_clicked, clear_clicked, default_clicked, random_clicked, test_vars
                )
        ...
        for var in template_vars:
            if var == 'candidates':
                ...
                if candidates_input.strip():
                    test_vars[var] = [item.strip() for item in candidates_input.split(',') if item.strip()]
            else:
                # 普通变量
    ...
    def _handle_validation_buttons(self, validate_clicked: bool, clear_clicked: bool,
                                 default_clicked: bool, random_clicked: bool, test_vars: Dict[str, Any]):
        """处理验证按钮点击"""
        if validate_clicked:
            ...
            try:
                ...
                with st.spinner("正在验证模板..."):
                    response = requests.post(
                        f"{self.api_url}/agents/{selected_agent}/prompts/validate",
                        json=test_vars,
                        timeout=30
                    )
