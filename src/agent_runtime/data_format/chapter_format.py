...
class ChapterNode(BaseModel):
    ...
    content_ids: List[str] = Field(default_factory=list)  # 关联的CQA内容ID
    related_cqa_items: List[CQAItem] = Field(default_factory=list)  # 关联的CQA案例对象
    ...
    def add_cqa_item(self, cqa_item: CQAItem) -> None:
        ...
        if cqa_item not in self.related_cqa_items:
            ...
            if cqa_item.cqa_id not in self.content_ids:
                self.content_ids.append(cqa_item.cqa_id)
    
    def add_cqa_id(self, cqa_id: str) -> None:
        """添加CQA案例ID"""
        if cqa_id not in self.related_cqa_ids:
            self.related_cqa_ids.append(cqa_id)
...
class ChapterStructure(BaseModel):
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any], max_level: int = 3) -> "ChapterStructure":
        """从字典创建章节结构"""
        structure = cls(max_level=max_level)
        
        def _parse_node(node_data: Dict[str, Any], parent_id: Optional[str] = None) -> None:
            node = ChapterNode(
                id=node_data.get("id", ""),
                ...
                content_ids=node_data.get("content_ids", []),
    ...
    def structure_str(self, show_cqa_info: bool = True) -> str:
        ...
        def _build_tree_display(node_id: str, indent: str = "") -> None:
            ...
            if show_cqa_info and node.related_cqa_items:
                lines.append(f"{indent}  关联CQA: {len(node.related_cqa_items)}个")
...
class ChapterResponse(BaseModel):
    ...
    content_mapping: Dict[str, List[str]] = Field(default_factory=dict)  # 节点ID到CQA内容ID的映射
