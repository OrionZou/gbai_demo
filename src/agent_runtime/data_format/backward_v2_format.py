...
class BackwardV2Request(BaseModel):
    ...
    qa_lists: List[QAList] = Field(
        ..., 
        description="Q&A二维列表，第一维区分多个多轮对话，第二维区分多轮对话中的多个Q&A"
    )
    chapter_structure: Optional[ChapterStructure] = Field(
        None, 
        description="可选的章节目录"
    )
    max_level: int = Field(3, description="章节目录最大层数")
    max_concurrent_llm: int = Field(15, description="最大并发数量")


class BackwardV2Response(BaseModel):
    """Backward V2 服务响应"""

    chapter_structure: ChapterStructure = Field(
        ..., 
        description="最终的章节目录结构"
    )
    ospa_list: List[OSPA] = Field(
        ..., 
        description="生成的OSPA列表"
    )
