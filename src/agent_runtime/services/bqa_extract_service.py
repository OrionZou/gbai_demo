...
class BQAExtractService:
    ...
    async def extract_bqa_from_conversations(
        ...
        total_extracted_bqas = sum(result.extracted_bqa_count for result in session_results)
        total_processing_time = int((time.time() - start_time) * 1000)

        operation_log.append(f"完成所有会话处理，总计: {total_original_qas} QA -> {total_extracted_bqas} BQA")
        operation_log.append(f"平均上下文依赖比例: {stats.avg_context_dependency_ratio:.2%}")

        # 构建处理摘要
        processing_summary = {
            "mode": request.context_extraction_mode,
            "concurrent_limit": request.max_concurrent_processing,
            "success_rate": len([r for r in session_results if r.extracted_bqa_count > 0]) / len(session_results),
            "stats": {
    ...
    def _analyze_context_dependency(
        ...
        with_background_count = sum(
            1 for bqa in bqa_list.items
            if bqa.background and bqa.background.strip()
        )
